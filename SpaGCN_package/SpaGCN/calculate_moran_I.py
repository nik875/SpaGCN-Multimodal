...
def Moran_I(genes_exp, x, y, k=5, knn=True):
    ...
    for i in genes_exp.columns:
        X_minus_mean = np.array(genes_exp[i] - np.mean(genes_exp[i]))
        ...
        I[i] = (len(genes_exp[i]) / np.sum(W)) * (Nom / Den)
    return I
...
def Geary_C(genes_exp, x, y, k=5, knn=True):
    ...
    for i in genes_exp.columns:
        ...
        Den = np.sum(np.multiply(X_minus_mean, X_minus_mean))
        C[i] = (len(genes_exp[i]) / (2 * np.sum(W))) * (Nom / Den)
    return C
